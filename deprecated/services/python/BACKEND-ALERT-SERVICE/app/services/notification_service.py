...
def _unwrap_secret(value: Optional[Union[str, SecretStr]]) -> Optional[str]:
    if value is None:
        return None
...
class NotificationService:
    ...
    def _send_email_notification(db: Session, alert: Alert) -> Optional[Notification]:
        ...
        try:
            ...
            smtp_username = _unwrap_secret(settings.SMTP_USERNAME)
            smtp_password = _unwrap_secret(settings.SMTP_PASSWORD)
    ...
    def _send_slack_notification(db: Session, alert: Alert) -> Optional[Notification]:
        ...
        try:
            slack_webhook_url = _unwrap_secret(settings.SLACK_WEBHOOK_URL)
            slack_channel = settings.SLACK_CHANNEL or "default"
            ...
            notification_data = NotificationCreate(
                ...
                metadata={"webhook_configured": bool(slack_webhook_url)}
            )
